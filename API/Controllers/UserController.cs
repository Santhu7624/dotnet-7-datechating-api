...
namespace API.Controllers
{
    ...
    public class UserController : BaseApiController
    {
        ...
        public UserController(IUnitOfWork uow, IMapper mapper, IPhotoService photoService, ILoggerService logger)
        {
            _uow = uow;
            _mapper = mapper;
        ...
        // {
        //     var result = await _uow.UserRepository.GetMembersAsync();
        //     return result;
        // }
        ...
        public async Task<ActionResult<PagedList<MemberDto>>> GetUsers([FromQuery] UserSpecParams paginationParams)
        {
            ...
            if (string.IsNullOrEmpty(User.GetUserName())) return null;

            var gender = await _uow.UserRepository.GetUserGender(User.GetUserName());        
            paginationParams.UserName = User.GetUserName();

            if(string.IsNullOrEmpty(paginationParams.Gender))
                paginationParams.Gender = gender == "male" ? "female" : "male";

            var result = await _uow.UserRepository.GetMembersAsync(paginationParams);
